...
class Net(nn.Module):
    ...
    def __init__(self, x_dim, factor, scope=None):
        ...
        # self.linear2 = nn.Linear(32, 32)  # Example: 1st added hidden layer
        # nn.init.kaiming_normal_(self.linear2.weight, mode='fan_in', a=1.)
        # nn.init.constant_(self.linear2.bias, 0.)

        # Add more hidden layers here
        # self.linear3 = nn.Linear(32, 10)  # Example: 1st added hidden layer
        # nn.init.kaiming_normal_(self.linear3.weight, mode='fan_in', a=1.)
        # nn.init.constant_(self.linear3.bias, 0.)
        
        self.fc_par_1 = nn.Linear(32, x_dim)
    ...
    def forward(self, x, scope=None, factor=None):
        ...
        h1 = self.fc_zip_1(x[0])[None] #x
        h2 = self.fc_zip_2(x[1])[None] #grad
        h3 = self.fc_zip_3(x[2])[None] #t

        h_sum = torch.relu(torch.sum(torch.cat([h1, h2, h3], dim=0), dim=0))

        h4 = torch.relu(self.linear(h_sum))
        # Pass through the new hidden layers
        #h4 = torch.relu(self.linear2(h4)) 
        #h4 = torch.relu(self.linear3(h4)) 
        ...
        return out1, out2, out3 # S,T,Q respectively