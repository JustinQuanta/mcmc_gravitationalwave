...
class Dynamics(nn.Module):
    ...
    def __init__(self,
        ...
        if hmc:
            ...
            self.XNet = net_factory(self.x_dim, scope='XNet', factor=2.0).to(device)  # net_factory is just a NN
            self.VNet = net_factory(self.x_dim, scope='VNet', factor=1.0).to(device)
    ...
    def _gen_mask(self, x): # May be redundant, but good for sanity check
